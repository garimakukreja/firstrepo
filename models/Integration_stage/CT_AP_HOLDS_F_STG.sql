...
ct_ap_holds_f_stg as (
  ...
  FROM 
    (
     SELECT 
    CONCAT(COALESCE(apholds_f_join_a.vendor_id, '0'), '~', COALESCE(apholds_f_join_a.vendor_site_id, '0')) AS supplier_id,
    apholds_f_join_a.set_of_books_id AS ledger_id,
    apholds_f_join_a.org_id_1 AS bu_id,
    apholds_f_join_a.legal_entity_id AS legal_entity_id,
    COALESCE(apholds_f_join_a.accts_pay_code_combination_id, 0) AS glcc_id,
    apholds_f_join_a.org_id AS org_id,
    CAST(TO_CHAR(apholds_f_join_a.invoice_date, 'YYYYMMDD') AS INT) AS invoice_dt_id,
    CAST(TO_CHAR(apholds_f_join_a.hold_date, 'YYYYMMDD') AS INT) AS hold_dt_id,
    CAST(TO_CHAR(apholds_f_join_a.gl_date, 'YYYYMMDD') AS INT) AS accounting_dt_id,
    COALESCE(apholds_f_join_a.inventory_item_id, 0) AS product_id,
    NULL AS supplier_contact,
    TO_CHAR(apholds_f_join_a.invoice_date, 'MON-YY') AS period_name,
    EXTRACT(YEAR FROM apholds_f_join_a.invoice_date) AS period_year,
    EXTRACT(MONTH FROM apholds_f_join_a.invoice_date) AS period_num,
    apholds_f_join_a.name AS ledger_name,
    apholds_f_join_a.invoice_id,
    apholds_f_join_a.invoice_num,
    apholds_f_join_a.description AS invoice_desc,
    ...
    CONCAT(
        apholds_f_join_a.invoice_id, '~',
        apholds_f_join_a.line_number, '~',
        apholds_f_join_a.hold_id, '~',
        COALESCE(apholds_f_join_a.hold_reason, '')
    ) AS integration_id,
    1000 AS datasource_num_id,
    'N' AS delete_flag
FROM (
SELECT 
    ...
    apholds_f_join_a.currency_code AS ledger_curr_code,
    apholds_f_join_a.displayed_field AS hold_name,
    apholds_f_join_a.requester_id AS requester_id,
    apholds_f_join_a.wf_status AS hold_wfapproval_status,
    apholds_f_join_a.displayed_field_1 AS hold_release_name,
    1 AS hold_cnt,
    apholds_f_join_a.invoice_amount,
...
FROM (
    ...
    FROM ap_invoices_all invoices_all
    JOIN ap_holds_all holds_all 
        ON holds_all.invoice_id = invoices_all.invoice_id
    JOIN ap_invoice_lines_all invoice_lines_all 
        ON invoice_lines_all.invoice_id = invoices_all.invoice_id
    LEFT JOIN po_headers_all po_headers_all 
        ON invoices_all.po_header_id = po_headers_all.po_header_id
    LEFT JOIN hz_parties hz_parties 
        ON invoices_all.party_id = hz_parties.party_id
    LEFT JOIN hz_party_sites hz_party_sites 
        ON invoices_all.party_site_id = hz_party_sites.party_site_id
    JOIN ap_hold_codes hold_codes 
        ON holds_all.hold_lookup_code = hold_codes.hold_lookup_code
    LEFT JOIN ap_hold_codes hold_codes1 
        ON holds_all.release_lookup_code = hold_codes1.hold_lookup_code
    JOIN gl_ledgers gl_ledgers 
        ON gl_ledgers.ledger_id = invoices_all.set_of_books_id
    JOIN ap_lookup_codes lookup_codes 
        ON holds_all.hold_lookup_code = lookup_codes.lookup_code
       AND lookup_codes.lookup_type = 'HOLD CODE'
    LEFT JOIN ap_lookup_codes lookup_codes1 
        ON holds_all.release_lookup_code = lookup_codes1.lookup_code

...
select 
  cast(coalesce(apholds_f_join_a.vendor_id, 0) as varchar)
  || '~' || 
  cast(coalesce(apholds_f_join_a.vendor_site_id, 0) as varchar)
  as supplier_id, 
  ...
  cast(apholds_f_join_a.invoice_id as varchar)
  || '~' || 
  cast(apholds_f_join_a.line_number as varchar)
  || '~' || 
  cast(apholds_f_join_a.hold_id as varchar)
  || '~' || 
...
from 
  (
    ...
    from 
      ...
      left join hz_party hz_party on invoices_all.party_id = hz_party.party_id 
      left join hz_party_site hz_party_site on invoices_all.party_site_id = hz_party_site.party_site_id 
      ...
      join gl_ledger gl_ledger on gl_ledger.ledger_id = invoices_all.set_of_books_id 
      ...
      and lookup_codes1.lookup_type = 'hold code'
       --where ( holds_all.last_update_date >= cast('$last_run_date$' as date) or invoices_all.last_update_date >= cast('$last_run_date$' as date) or invoice_lines_all.last_update_date >= cast('$last_run_date$' as date) )
        ) apholds_f_join_a
)
